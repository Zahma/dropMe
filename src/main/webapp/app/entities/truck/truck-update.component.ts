...
type SelectableEntity = IDriver | ITrip | ITransporterAccount;
        ...
        .subscribe((resBody: ITrip[]) => {
          if (!truck.tripId) {
            ...
            this.tripService
              .find(truck.tripId)
              .pipe(
                map((subRes: HttpResponse<ITrip>) => {
                  return subRes.body ? [subRes.body].concat(resBody) : resBody;
                })
              )
              .subscribe((concatRes: ITrip[]) => (this.trips = concatRes));
          }
        });
