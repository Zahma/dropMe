...
export class TransporterAccountUpdateComponent implements OnInit {
  ...
  updateForm(transporterAccount: ITransporterAccount): void {
    this.editForm.patchValue({
      ...
      managerFName: transporterAccount.managerFName,
      managerLName: transporterAccount.managerLName,
      ...
      activated: transporterAccount.activated,
  ...
  setFileData(event: Event, field: string, isImage: boolean): void {
    this.dataUtils.loadFileToForm(event, this.editForm, field, isImage).subscribe(null, (err: JhiFileLoadError) => {
      this.eventManager.broadcast(
        new JhiEventWithContent<AlertError>('dropMeApp.error', { ...err, key: 'error.file.' + err.key })
      );
    });
  }
  ...
  private createFromForm(): ITransporterAccount {
    return {
      ...
      managerFName: this.editForm.get(['managerFName'])!.value,
      managerLName: this.editForm.get(['managerLName'])!.value,
